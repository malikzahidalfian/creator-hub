...
AI Image membuka pilihan `GPT-5.5 + GPT Image 1.5`: GPT-5.5 menyusun prompt visual melalui `/api/generate`, kemudian `/api/generate-image` membuat gambar memakai ID katalog 1inference `venice-gpt-image-1.5`. Kedua permintaan memakai API key browser dan saldo provider. GPT-5.5 tidak dikirim sebagai model ke endpoint gambar. Pilihan GPT Image langsung melewati tahap konsep; model lain tetap mengikuti pilihan pengguna. Respons URL dan base64 didukung, dan kegagalan tahap konsep menghentikan proses tanpa memanggil model gambar. Waktu tunggu gambar 150 detik di proxy, dengan batas fungsi Vercel dan browser 180 detik; deployment memakai Fluid compute. Ketersediaan model di katalog tidak membuktikan akses API key pengguna.

Threads Artikel membatasi `max_completion_tokens` ke 4096, konsep gambar ke 2048, dan rekomendasi gaya tetap 400. Batas mencakup penalaran serta teks keluaran. Percobaan ulang gambar dengan prompt dan API key yang sama memakai ulang konsep yang sudah berhasil dibuat dari memori halaman. Respons 402 tetap merupakan kegagalan pembayaran provider: aplikasi menampilkan tahap, model, kode dan ID permintaan jika tersedia, tanpa menganggap saldo akun pasti habis atau mengganti model otomatis. Batas token dan penanganan error tidak menjamin masalah billing provider terselesaikan; diagnosis akun memerlukan respons langsung memakai API key terkait.
